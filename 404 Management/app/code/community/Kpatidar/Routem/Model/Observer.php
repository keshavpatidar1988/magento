...
class Kpatidar_Routem_Model_Observer {
    public function routeManagement(Varien_Event_Observer $observer) {
 ...
 // Redirect request if found 404
    ...
    public function checkModuleStatus() {
        ...
        return Mage::getStoreConfig('routem/routemstatus/status', $storeId);
    }
    
    // Check 404 not found page status in admin
    public function check404ModuleStatus() {
        $storeId = Mage::app()->getStore()->getStoreId();
        return Mage::getStoreConfig('routem/routemstatus/status404', $storeId);
    }
